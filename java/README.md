...
# 어노테이션
* 어노테이션이란? 프로그램 소스 코드안에 다른 프로그램을 위한 정보를 미리 약속된 형식으로 포함시킨 것이다.
* @Override, @Test 이런 형태로 많이 봤을것이다. 
...
* 쓰레드란? 프로세스의 자원을 이용해서 실제로 작업을 수행하는 것이다.
* 싱글 쓰레드 = 자원 + 쓰레드 , 멑리 쓰레드 = 자원 + 쓰레드 + 쓰레드 ....
...
## 싱글쓰레드와 멀티쓰레드
* 멀티쓰레드를 쓰더라도 경우에 맞게 써야한다. 멀티 쓰레드는 쓰레드간의 작업 전환에 시간이 걸리기 때문에 싱글쓰레드를 쓸 때 보다 시간이 더 걸릴 수 있다.
* 단순히 CPU만을 사용하는 계산작업이라면 멀티쓰레드 보다는 싱글쓰레드가 더 효율적이다.
 
## 쓰레드 기능 정리
 * sleep : 일정시간동안 쓰레드 멈춤
 * interrupt, interrupted: 쓰레드의 작업을 취소한다.
 * suspend로 정지 시키고 resume으로 호출
 * yield: 다른 쓰레드에게 양보
...
* 만약 특정 쓰레드가 객체의 락을 오래 가지면 어떻게 될까? 다른 쓰레드는 해당 객체의 락이 풀릴 때까지 기달려야한다.
* 이런 상황을해결하기 위해 만든것이 wait(), notify()다.
* wait: 호출하면 쓰레드가 락을 반납하고 기다리게 한다. 


# 람다(Lamda)
* 람다식이란 메서드를 하나의 식으로 표현한 것이다. 메서드를 람다식으로 표현하면 메서드의 이름과 반환값이 없어지므로, 익명 함수라고도 부른다.
```
() -> (int)(Math.random())
```

## 람다식 작성
* 람다식은 메서드에서 이름과 반환타입을 제거하고 선언부와 몸통으로 나뉜다.
```
(매개변수 선언) -> {
    문장들
}
...
* 서버는 서비스를 제공하는 컴퓨터(service provider)이고 클라이언트는 서비스를 사용하는 컴퓨터(service user)이다.
* 서비스는 서버가 클라이언트로부터 요청받은 작업을 처리하며 그 결과를 제공하는 것. 서버가 제공하는 서비스의 종류에 따라 파일 서버, 메일서버, 어플리케이션 서버 등이 있다.
* 네트워크를 구성할 때 전용서버를 두는 것을 서버기반모델(server-based model)이라 하고 별도의 전용서버 없이 각 클라이언트가 서버역할을 동시에 수행하는 것을 P2P모델(peer-to-peer)라고 한다.	
